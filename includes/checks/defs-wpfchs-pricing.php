<?php
/**
 * WPFactory Catalog Health Scanner for WooCommerce - Check Definitions - Pricing Sanity
 *
...
return array(
	...
	array(
		'id'          => 'price_zero',
		'group'       => 'selling',
		'label'       => __( 'Price of zero on a product not marked as free', 'wpfactory-catalog-health-scanner-for-woocommerce' ),
		'explanation' => __( 'This product sells for nothing. If that is not deliberate, every order is pure loss.', 'wpfactory-catalog-health-scanner-for-woocommerce' ),
		'severity'    => 'high',
	...
	array(
		...
		'group'       => 'selling',
		'label'       => __( 'Price far outside the category median', 'wpfactory-catalog-health-scanner-for-woocommerce' ),
		'explanation' => __( 'A price several times above or below everything comparable is usually a misplaced decimal point.', 'wpfactory-catalog-health-scanner-for-woocommerce' ),
		'severity'    => 'high',
		'pass'        => 'catalog',
		'check'       => function () {
			global $wpdb;
				...
				if ( $judged_in > 0 && $outlier_in === $judged_in ) {
					$results[ $product_id ] = array(
						...
						'value'      => sprintf(
							...
							__( 'Price %1$s vs category median %2$s', 'wpfactory-catalog-health-scanner-for-woocommerce' ),
	...
	array(
		...
		'group'       => 'selling',
		'label'       => __( 'Price with more decimals than the store currency supports', 'wpfactory-catalog-health-scanner-for-woocommerce' ),
		'explanation' => __( 'The displayed price is rounded, so what customers see is not what is charged.', 'wpfactory-catalog-health-scanner-for-woocommerce' ),
		'severity'    => 'medium',
		'check'       => function ( $product ) {
	...
	array(
		'id'          => 'cog_missing',
		'label'       => __( 'Cost of goods missing', 'wpfactory-catalog-health-scanner-for-woocommerce' ),
		'explanation' => __( 'Without a cost you cannot see margin, so you cannot see which products lose money.', 'wpfactory-catalog-health-scanner-for-woocommerce' ),
		'severity'    => 'medium',
		'group'       => 'cog',
		'fix'         => 'set_cog_percent',
		'fix_type'    => 'bulk',
		'check'       => function ( $product ) use ( $wpfchs_get_cost ) {
			return ( null === $wpfchs_get_cost( $product ) ? __( 'No cost set', 'wpfactory-catalog-health-scanner-for-woocommerce' ) : false );
		},
	),

	array(
		'id'          => 'cog_above_price',
		'label'       => __( 'Cost of goods higher than selling price', 'wpfactory-catalog-health-scanner-for-woocommerce' ),
		'explanation' => __( 'You lose money on every unit sold.', 'wpfactory-catalog-health-scanner-for-woocommerce' ),
		'severity'    => 'critical',
		'group'       => 'cog',
		'check'       => function ( $product ) use ( $wpfchs_get_cost ) {
			...
			if ( null !== $cost && '' !== $price && $cost > (float) $price ) {
				return sprintf(
					...
					__( 'Cost %1$s vs price %2$s', 'wpfactory-catalog-health-scanner-for-woocommerce' ),
	...
	array(
		...
		'label'       => __( 'Margin below your minimum threshold', 'wpfactory-catalog-health-scanner-for-woocommerce' ),
		'explanation' => __( 'This product sells below the margin floor you set in settings.', 'wpfactory-catalog-health-scanner-for-woocommerce' ),
		'severity'    => 'high',
		'group'       => 'cog',
		'applies'     => function () {
		...
		'check'       => function ( $product ) use ( $wpfchs_get_cost ) {
			...
			if ( $margin < $threshold ) {
				return sprintf(
					...
					__( '%1$s%% margin, minimum is %2$s%%', 'wpfactory-catalog-health-scanner-for-woocommerce' ),
