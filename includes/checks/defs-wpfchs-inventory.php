<?php
/**
 * Catalog Health Scanner for WooCommerce - Check Definitions - Inventory
 *
...
return array(
	...
	array(
		...
		'check'       => function ( $product ) {
			return ( '' === $product->get_sku( 'edit' ) ? __( 'No SKU', 'catalog-health-scanner-for-woocommerce' ) : false );
		},
	),
		...
		'check'       => function ( $product ) {
			...
			if ( $missing > 0 ) {
				return sprintf(
					...
					__( '%d variation(s) without a SKU', 'catalog-health-scanner-for-woocommerce' ),
					$missing
				);
			}
			return false;
	...
	array(
		'id'          => 'negative_stock',
		'label'       => __( 'Negative stock quantity', 'catalog-health-scanner-for-woocommerce' ),
		'explanation' => __( 'Stock has gone below zero, which usually means orders were placed that could not be fulfilled.', 'catalog-health-scanner-for-woocommerce' ),
		'severity'    => 'high',
		'check'       => function ( $product ) {
	...
	array(
		...
		'label'       => __( 'Stock quantity set while stock management is off', 'catalog-health-scanner-for-woocommerce' ),
		'explanation' => __( 'These numbers are stored but ignored, which misleads anyone reading them.', 'catalog-health-scanner-for-woocommerce' ),
		'severity'    => 'medium',
	...
	array(
		...
		'label'       => __( 'Stock status contradicting stock quantity', 'catalog-health-scanner-for-woocommerce' ),
		'explanation' => __( 'Products showing in stock with zero units, or out of stock with units available.', 'catalog-health-scanner-for-woocommerce' ),
		'severity'    => 'high',
		...
		'check'       => function ( $product ) {
			...
			if ( $qty > 0 && 'outofstock' === $status ) {
				return sprintf(
					...
					__( 'Out of stock with %d units', 'catalog-health-scanner-for-woocommerce' ),
					$qty
				);
			}
			if ( $qty <= 0 && 'instock' === $status && ! $product->backorders_allowed() ) {
				return sprintf(
					/* translators: %d: stock quantity. */
					__( 'In stock with %d units', 'catalog-health-scanner-for-woocommerce' ),
					$qty
				);
			}
			return false;
